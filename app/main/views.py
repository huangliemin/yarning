# -*- coding: UTF-8 -*-
from flask import render_template, redirect, url_for, flash, request, make_response, current_app, abort
from . import main
...
def index():
	...
	if show_followed:
		...
		query = Post.query
	pagination = query.order_by(Post.timestamp.desc()).paginate(page,per_page=current_app.config['YARNING_POSTS_PER_PAGE'],error_out=False)
...
def post(id):
	...
	page = request.args.get('page',1,type=int)
	if page == -1:
		page = (post.comments.count()-1) / current_app.config['YARNING_COMMENTS_PER_PAGE'] + 1
	pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(page,per_page=current_app.config['YARNING_COMMENTS_PER_PAGE'],error_out=False)
...
def moderate():
	page = request.args.get('page',1,type=int)
	pagination = Comment.query.order_by(Comment.timestamp.desc()).paginate(page,per_page=current_app.config['YARNING_COMMENTS_PER_PAGE'],error_out=False)
	comments = pagination.items
